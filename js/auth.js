...
class AuthManager {
  ...
  async initializeWithRetry() {
    while (this.retryCount < this.maxRetries && !this.isInitialized) {
      try {
        ...
        this.retryCount++;
        console.error(
          `❌ AuthManager initialization attempt ${this.retryCount} failed:`,
          error
        );

        if (this.retryCount < this.maxRetries) {
          await new Promise((resolve) =>
            setTimeout(resolve, this.retryCount * 1000)
          );
        } else {
          console.error(
            "💥 AuthManager initialization failed after all retries"
          );
  ...
  async initSupabase() {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ Session check failed, but client is initialized:",
          sessionError
        );
        // Don't throw here as the client might still work for other operations
      }
    } catch (clientError) {
  ...
  setupAuthStateListener() {
    if (!this.supabase || !this.supabase.auth) {
      console.error(
        "Cannot setup auth state listener: Supabase client not initialized"
      );
      return;
    }
  ...
  // Utility method to validate Supabase client
  ...
  handleAuthLogout() {
    ...
    if (this.isProtectedPage()) {
      ...
      window.location.href = `${baseUrl}/auth/login.html?returnUrl=${encodeURIComponent(
        currentPath
      )}`;
    }
  }
  ...
  async handleSignup(event) {
    ...
    try {
      ...
      if (authData.user && !authData.session) {
        ...
        this.showSuccess(
          "অ্যাকাউন্ট তৈরি সফল হয়েছে! আপনার ইমেইল চেক করে লিংকে ক্লিক করুন।"
        );
      } else {
  ...
  async handleLogin(event) {
    ...
    if (!this.isInitialized || !this.validateSupabaseClient()) {
    ...
    try {
      ...
      if (error.message.includes("Invalid login credentials")) {
        ...
        errorMessage =
          "আপনার ইমেইল যাচাই করুন। ইমেইলে পাঠানো লিংকে ক্লিক করুন।";
  ...
  async logout() {
    if (!this.validateSupabaseClient()) {
      // Still clear local data even if Supabase client is not available
      localStorage.removeItem("mental_health_user");
  ...
  getErrorMessage(error) {
    const errorMessages = {
      ...
      "Email not confirmed":
        "আপনার ইমেইল নিশ্চিত করুন। ইমেইলে পাঠানো লিংকে ক্লিক করুন।",
      "User already registered":
        "এই ইমেইল ইতিমধ্যে নিবন্ধিত। লগইন করার চেষ্টা করুন।",
      ...
      "Email rate limit exceeded":
        "অনেক চেষ্টা হয়েছে। ৫ মিনিট পর আবার চেষ্টা করুন।",
