...
class MentalHealthApp {
  constructor() {
    ...
    this.supabase = null;
    this.sessionId = null; // Add missing session ID
  ...
  async initializeAsync() {
    try {
      ...
      if (!window.configManager) {
        // Fallback: wait a bit for config manager to load
      ...
      if (typeof window.supabase !== "undefined") {
        ...
        if (this.supabase && this.supabase.auth) {
          ...
          this.showErrorMessage(
            "সিস্টেম ইনিশিয়ালাইজেশন ব্যর্থ। পেজ রিফ্রেশ করুন।"
          );
        }
      } else {
        console.error("Supabase library not loaded");
        this.showErrorMessage("লাইব্রেরি লোড হয়নি। পেজ রিফ্রেশ করুন।");
      }
    } catch (error) {
      console.error("Failed to initialize Supabase:", error);
      this.showErrorMessage("সংযোগ সমস্যা। পেজ রিফ্রেশ করুন।");
    }
  }
  ...
  async checkAuthStatus() {
    if (!this.supabase || !this.supabase.auth) {
      console.warn("Supabase client not properly initialized for auth check");
      return;
    }
    ...
    try {
      ...
      this.showErrorMessage("অথেনটিকেশন সমস্যা। পেজ রিফ্রেশ করুন।");
    }
  }
  ...
  displayWelcomeMessage() {
    if (this.chatDisplay) {
      const welcomeMessage =
        "হ্যালো! আমি আপনার মানসিক স্বাস্থ্য সহকারী। আজ আপনার কেমন লাগছে? আপনি নির্দ্বিধায় আপনার মনের কথা বলতে পারেন।";
  ...
  setupErrorHandling() {
    window.addEventListener("error", (event) => {
      ...
      this.showErrorMessage(
        "একটি অপ্রত্যাশিত সমস্যা হয়েছে। পেজ রিফ্রেশ করে আবার চেষ্টা করুন।"
      );
    });
  ...
  announceMessage(message, sender) {
    const announcement = sender === "user" ? "আপনি বলেছেন: " : "AI বলেছে: ";
  ...
  async callGeminiAPI(prompt, history = []) {
    ...
    if (!this.API_KEY || this.API_KEY.trim() === "") {
      throw new Error(
        "API key not configured. Please add AI_API_KEY to your .env file."
      );
    }

    const systemPrompt = `You are a friendly and empathetic mental health assistant for Bangladeshi people, especially students. You should:
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Make app globally accessible for reference
